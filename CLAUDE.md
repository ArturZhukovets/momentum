...
Russian strings live only in `texts.py`, never inline them.
...
- **`db/repo.py` holds ALL SQL** (hand-written, no ORM), returning frozen dataclasses
  (`Workout`, `WorkoutPoint`, `UserRow`). Every workout query is scoped by `user_id` so
  ids can't be poked cross-user — preserve this on any new query.
...
Typed `CallbackData` factories live in `keyboards.py` (`ActionCB`,
...
- Dates stored as `'YYYY-MM-DD'` local-date **text**, converted at the repo boundary
  (`ISO_DATE`, `_to_date`). `kind` is constrained to `'cardio'|'strength'` in schema.
